...
class GaussianNoise(nn.Module):
    ...
    def forward(
        ...
        if torch.rand(1) < self.p:
            ...
            img_pil = np.array(transform(image), dtype='uint8')
            #mask = np.array(img2, dtype='uint8')
            trans = A.GaussNoise(p=1)
            augmented = trans(image=img_pil)#, mask)
            img_noise = Image.fromarray(augmented['image'])
            transform = PILToTensor()
            img_noise, _ = transform(img_noise)
