...
def fuse_predictions(tensor, img_shape=None, arg=None, is_inchannel=False, is_testing=True):
    fuse_name = 'fused'
    av_name = 'avg'
    tensor2=None
    tmp_img2 = None
    ...
    for i_shape in image_shape:
        ...
        image_list.append(fuse)
        idx += 1
    return image_list

def save_image_batch_to_disk(tensor, output_dir, file_names, img_shape=None, arg=None, is_inchannel=False, is_testing=True):
...
def visualisePolygons(list_of_polygons, image):
    """
    Simple function for visualising polygons,
    polygons should be not georeferenced,
    they should be in pixel coordinates
    """
    org = image.copy()

    for s in list_of_polygons:
        poly = s[0]['coordinates']
        rng = np.random.default_rng()
        new_color = (rng.choice(255, size = 3, replace=False))
        new_color = (int (new_color[0]), int (new_color[1]), int (new_color[2]))
        new_poly = []
        for in_poly in poly:
            new_poly.append(np.array(in_poly, dtype='int32'))
        cv2.polylines(org, new_poly, False , new_color, 1)
    return org
